...
- **Godot4.5+**, to be able to test the plugin within a Godot project. The minimum required Godot version is 4.5. The plugin is tested with 4.6 as well.
...
Cross-compiling to Android target devices is supported in a BETA state. Required software to enable cross compilation:
- **Android Studio** - Use the Android Studio Flamingo to install the additional requires toolchain 
- **Android SDK** - the installed components and their versions should be:
  - Android SDK Platform: 36
  - SDK Build Tools: 37.0.0
  - NDK (Side-by-side): 29.0.x
  - CommandLine Tools: 20.0
  - CMake: 4.1.2
  - Android SDK Platform Tools: 37.0.0

To export the Godot project using this plugin to an Android device, the `Gradle` build is required. After installing the corresponding Android build template the `GodotApp.java` file
from `./templates/android/build/src/main/java/com/godot/game` folder is required to replace the existing one in the `android/build/...` folder of the build template used for this project.
This is required as long as the openUSD does not natively support, scanning anfd loading of it's plugInfo.json configuration files from the Android specific package files. The implementation
ensures, that the required files are copied into the user folder and that the openUSD library is configured to search in this very folder for it's configuration files.
...
```bash
checked_out_repo_dir $>scons platform=android ANDROID_NDK_ROOT=C:\Users\<your-user>\AppData\Local\Android\Sdk\ndk\29.0.14206865 target=template_release    
```
---------------------
